...
def vague_count(para):
    #模糊查询
    student_count = Student.query.filter(Student.stu_name.like(para),or_(Student.stu_enter_time.like(para)),or_(Student.industry_id.like(para)),or_(Student.stu_company.like(para)),or_(Student.stu_position.like(para))).count()

    return student_count

def vague_first(para):
    #查询到一个
     student_first = Student.query.filter(Student.stu_name.like(para),or_(Student.stu_enter_time.like(para)),or_(Student.industry_id.like(para)),or_(Student.stu_company.like(para)),or_(Student.stu_position.like(para))).first()
     return student_first

def vague_all(para):
     #查询到多条
     student_all = Student.query.filter(Student.stu_name.like(para),or_(Student.stu_enter_time.like(para)),or_(Student.industry_id.like(para)),or_(Student.stu_company.like(para)),or_(Student.stu_position.like(para))).all()
     return student_all
