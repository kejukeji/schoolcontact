...
def select_student():
    ...
    student_selected_count = db.query(StudentsClass)\
        .join(IndustryClass).filter(StudentsClass.stu_enter_time==enter_time and IndustryClass.trade ==trade).count()
    if student_selected_count > 1:
        student_selected =  db.query(StudentsClass)\
        .join(IndustryClass).filter(StudentsClass.stu_enter_time==enter_time and IndustryClass.trade ==trade).all()
    else:
         student_selected = db.query(StudentsClass)\
        .join(IndustryClass).filter(StudentsClass.stu_enter_time==enter_time and IndustryClass.trade ==trade).first()
